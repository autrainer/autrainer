...
class TestDatasetConfigurations:
    @pytest.mark.parametrize("name, config", load_configurations("dataset"))
    def test_configurations(self, name: str, config: Dict[str, Any]) -> None:
        ...
        assert config.get("transform"), (
            f"{name}: Missing transform configuration"
        )
        assert config["transform"].get("type"), (
            f"{name}: Missing transform type"
        )
