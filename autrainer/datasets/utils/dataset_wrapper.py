...
class SSLDatasetWrapper(torch.utils.data.Dataset):
    def __init__(
        ...
        """Wrapper around torch.utils.data.Dataset for SSL.

        Returns features as targets, optionally augmented.
        ...
        Args:
            ...
            file_type: File type of the features. If `None`,
                ...
                with different formats. Defaults to `None`.
            transform: Transform to apply to the features. Defaults to None.
            target_transform: Transform to apply to the features
                that should be returned as targets. Defaults to `None`.
                Applied independently of `transform`.
    ...
    def __getitem__(self, item: int) -> DataItem:
        ...
        if self.transform is not None:
            features = self.transform(data, index=item).float()

        target = data
        if self.target_transform is not None:
            target = self.target_transform(target, index=item).float()

        return DataItem(features=features, target=target, index=item)
