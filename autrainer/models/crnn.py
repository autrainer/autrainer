...
class CRNN(AbstractModel):
    def __init__(
        ...
        pooling: Union[List[int], List[List[int]]] = [2, 4],
        activation: str = "Relu",
        hidden_size: int = 64,
        n_layers_rnn: int = 1,
        attention: bool = True,
    ...
    def forward(
        self, features: torch.Tensor, return_pre_threshold: bool = False
    ) -> Union[torch.Tensor, tuple]:
        x = self.embeddings(features)
        x = self.dropout_layer(x)
        strong = self.fc(x)
        strong = torch.sigmoid(strong)
        return strong
